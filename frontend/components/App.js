...
if (typeof window !== "undefined") {
  createAppKit({
    ...
    networks: [{ id: 56, name: "BNB Smart Chain", nativeCurrency: { name: "BNB", symbol: "BNB", decimals: 18 }, rpcUrls: { default: { http: ["https://bsc-dataseed1.binance.org"] } }, blockExplorers: { default: { name: "BscScan", url: "https://bscscan.com" } } }],
    projectId: "a76badfdf543ada7db3fe70ff41921cb",
    metadata: { name: "LookaWin", description: "Loterie décentralisée", url: "https://looka.win", icons: ["https://looka.win/favicon.svg"] },
    features: { analytics: false, email: false, socials: false },
    themeMode: "dark",
    themeVariables: { "--w3m-accent": "#b48eef", "--w3m-border-radius-master": "22px" },
  });
}
...
function App() {
  ...
  useEffect(() => { if (isConnected && address) fetchMyTickets(); else setMyTickets([]); }, [isConnected, address, fetchMyTickets]);
  ...
  const copyReferral = () => {
    if (!address) { showToast(lang==="fr"?"Connectez votre wallet":"Connect your wallet first", false); return; }
  ...
  const buyTickets = async () => {
    ...
    try {
      ...
      fetchState(); fetchMyTickets();
    } catch(e) {
      showToast(e?.reason || e?.message || "Error", false);
    }
    setLoading(false);
  };
  ...
  const activeT = tabs.find(t=>t.key===activeTab)||tabs[0];
  ...
  return (
    <>
      ...
      <style>{`
        .prize-hero { background:var(--bg); border:1px solid var(--bg); border-radius:var(--radius); padding:24px 18px 18px; margin-bottom:12px; text-align:center; position:relative; overflow:hidden; }
        .prize-hero::before { content:""; position:absolute; top:-50px; left:50%; transform:translateX(-50%); width:220px; height:220px; background:radial-gradient(circle,rgba(245,197,24,0.13) 0%,transparent 60%); pointer-events:none; }
        .prize-amount { font-family:var(--font); font-weight:800; font-size:clamp(2.2rem,9vw,3.4rem); color:var(--gold); line-height:1; letter-spacing:-1px; word-break:break-all; position:relative; z-index:1; }
        ...
        .ticket-status-lost { background:rgba(183,189,198,0.12); color:var(--muted); border-radius:8px; padding:3px 9px; font-size:0.68rem; font-weight:700; }
        .btn-connect-wallet { width:100%; padding:13px; background:var(--gold); border:none; border-radius:22px; color:#000; font-family:var(--font); font-weight:800; font-size:0.95rem; cursor:pointer; margin-top:4px; transition:background 0.15s; }
        .btn-connect-wallet:hover { background:var(--gold2); }
        .winner-item { display:flex; justify-content:space-between; align-items:center; padding:10px 13px; background:var(--surface2); border:1px solid var(--border); border-radius:22px; margin-bottom:6px; }
        ...
        .btn-disconnect { height:33px; width:33px; background:var(--surface2); border:1px solid var(--border); border-radius:22px; color:var(--muted); font-size:0.75rem; cursor:pointer; display:flex; align-items:center; justify-content:center; transition:all 0.15s; }
        .btn-disconnect:hover { background:rgba(255,107,107,0.15); border-color:#ff6b6b; color:#ff6b6b; }
        .wallet-chip { display:flex; align-items:center; gap:6px; padding:0 10px 0 8px; height:33px; background:var(--surface2); border:1px solid var(--border); border-radius:99px; font-size:0.78rem; font-weight:700; color:var(--text); }
        .wallet-dot { width:7px; height:7px; border-radius:50%; background:var(--green); }
        .toast { position:fixed; bottom:24px; left:50%; transform:translateX(-50%); padding:10px 20px; border-radius:22px; font-family:var(--font); font-weight:700; font-size:0.85rem; z-index:999; white-space:nowrap; box-shadow:0 4px 20px rgba(0,0,0,0.3); }
      ...
      <header className="app-header">
        ...
        <div className="header-actions">
          ...
          {isConnected && address ? (
            <>
              <div className="wallet-chip"><span className="wallet-dot"/>{address.slice(0,6)}&#8230;{address.slice(-4)}</div>
              <button className="btn-disconnect" onClick={()=>open({view:"Account"})} title="Account">&#9881;</button>
            </>
          ) : (
            <button className="btn-connect-header" onClick={()=>open()}>{t("connect_wallet")}</button>
          )}
        </div>
      </header>
      ...
      <main style={{maxWidth:480, margin:"0 auto", padding:"18px 14px 60px"}}>
        ...
        <div className="card a2">
          ...
          <div className="jackpot-val">
            <div className="jackpot-num">${activeT.value||"0.00"}</div>
        ...
        <div className="card a3">
          ...
          <div className="cost-row">
            ...
            <span className="cost-val">{quantity*2} USDT</span>
          </div>
            ...
            myTickets.length===0?(
              ...
              myTickets.map((tk,i)=>(
                <div key={i} className="ticket-item">
                  <div>
                    <p className="ticket-round" style={{margin:0, fontSize:"0.69rem"}}>
                      <a href={"https://bscscan.com/tx/" + (tk.txHash || tk.id)} target="_blank" rel="noopener noreferrer" style={{color:"var(--purple)",textDecoration:"none", fontWeight:"bold", fontSize:"0.69rem"}}>
                        #{tk.block || tk.round}
                      </a>
